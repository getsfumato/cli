...
fn assemble_page(request: PageAssemblyRequest<'_>) -> Result<AssembledPage> {
    ...
    // The stylesheet is written once, when the theme is imported, and nothing rewrites
    // it — so a theme installed before the token vocabulary existed still ships only the
    // six aliases. Deriving the rest from the manifest here is what makes the fix reach
    // themes already on disk, and only names the stylesheet leaves undefined are added,
    // so whatever it does declare stays authoritative.
    let missing_tokens = undeclared_token_properties(&theme_css, &request.theme.manifest);
...
/// A `:root` block for the manifest tokens the theme stylesheet never declares.
///
/// Returns an empty string when the stylesheet already covers everything, so a theme
/// regenerated by a current build emits no second block at all.
fn undeclared_token_properties(
    theme_css: &str,
    manifest: &sfumato_core::themes::ThemeManifest,
) -> String {
    let declared = declared_custom_properties(theme_css);
    ...
    for (name, value) in &manifest.tokens.fonts {
        if !declared.contains(&format!("--font-{name}")) {
...
///
/// Scanned rather than parsed: the question is only which names are already spoken for,
/// and a declaration is always `--name` followed by a colon.
fn declared_custom_properties(css: &str) -> BTreeSet<String> {
    let mut names = BTreeSet::new();
    let bytes = css.as_bytes();
    let mut index = 0;
    while let Some(found) = css[index..].find("--") {
        ...
        // Only a declaration counts; `var(--name)` is a use, and the difference is the
        // colon that follows.
        let mut after = end;
